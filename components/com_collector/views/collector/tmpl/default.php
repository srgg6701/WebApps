...
//var_dump("<h1>array:</h1><pre>",$this,"</pre>");?>
...
<table cellpadding="8" cellspacing="0" id="tblCollector" onClick="checkPatchBoxes(this);">
  <tr>
...
function checkPatchBoxes(table){
	try{
		//alert(event.target);
		var tBox=(navigator.appName.indexOf("Internet Explorer")==-1)? event.target:event.srcElement;
			...
			if (f>0){
				...
				for(k=0;k<f;k++){
					...
					if (k>0){
						...
						if (boxes.item(k).checked==true) bc++;
					}
				}
				var fm=(f-1);
