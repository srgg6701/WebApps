...
$myAntiSpamQuestion = $params->get('anti_spam_q', 'Сколько у обычного человека глаз?');
...
if ($user->get('email')) $CORRECT_EMAIL=$user->get('email');
// print email input
...
if ($enable_anti_spam) {
  if ($anti_spam_position == 1) {
    print '<tr><td colspan="2"><div class="bold">Ответьте на вопрос:</div>' . $myAntiSpamQuestion . ': <input class="rapid_contact inputbox ' . $mod_class_suffix . '" type="text" name="rp_anti_spam_answer" size="' . $emailWidth . '" value="'.$CORRECT_ANTISPAM_ANSWER.'"/></td></tr><tr>'.$emptycell.'<td></td></tr>' . "\n";
  }
}
// print button
print '<tr><td colspan="2"><button class="rapid_contact button ' . $mod_class_suffix . '" type="submit" style="width: ' . $buttonWidth . '%"/>'.$buttonText.'</button></td></tr></table></form></div>' . "\n";?>
</div><?
return true;
