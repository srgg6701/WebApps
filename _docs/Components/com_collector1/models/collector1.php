...
class Collector1ModelCollector1 extends JModel
{
	...
	function addCollection(){
		...
		if ($user->get('guest')==1) { //SDebug::dOutput("guest","h1");
...
FROM #__webapps_site_types ORDER BY id DESC";
	...
	function preparePreOrderDataValues(	//&$table,
		...
		/*if (!$post_collection['collections_ids']&&!$post_collection['orders_ids']){
			JMail::sendErrorMess("Не получены ни collections_ids, ни orders_ids...",$subject);
			return false;
		}
		if ($post_collection['collections_ids']&&$post_collection['orders_ids']){
			JMail::sendErrorMess("Получены оба объекта вместо одного -- и collections_ids, и orders_ids...",$subject);
			return false;
		}*/
		//добавить извлечённый ранее id последней коллекции или заказа:
		//if ($post_collection['collections_ids']) 
		...
		//$order_obj_type='collections_ids';
		//elseif ($post_collection['orders_ids']) $order_obj_type='orders_ids';
 ...
 WHERE `session_id` = '".session_id()."' OR email = '$post_collection[email]'";
		...
		if (empty($result)) {
			...
			//$table=
			self::preparePreOrderDataValues(/*$table,$arrPostData,$post_collection,*/$last_site_id);
			//Добавить данные в таблицу и проверить состояние:
			//SErrors::afterTable($table);
			...
			//}else{ 		
				//установить значения полей:
				//$table=
				self::preparePreOrderDataValues(/*$table,$arrPostData,$post_collection,*/$result['collections_ids'].','.$last_site_id,$result['id']);
